...
# 定义MERGE_OPTIONS（从原始代码中提取）
MERGE_OPTIONS = {
    1: {"portrait": (1, 1), "landscape": (1, 1)},  # 单张图片
    2: {"portrait": (2, 1), "landscape": (1, 2)},
    3: {"portrait": (3, 1), "landscape": (1, 3)},
    4: {"portrait": (2, 2), "landscape": (2, 2)},
    5: {"portrait": (3, 2), "landscape": (2, 3)},
    6: {"portrait": (2, 3), "landscape": (3, 2)},
    7: {"portrait": (3, 3), "landscape": (3, 3)},
    8: {"portrait": (4, 2), "landscape": (2, 4)},
    9: {"portrait": (3, 3), "landscape": (3, 3)},
    10: {"portrait": (5, 2), "landscape": (2, 5)}
}
...
def calculate_merged_size(batch_imgs, rows, cols, spacing):
    ...
    col_widths = []
    for i in range(cols):
        col_imgs = batch_imgs[i::cols] if i < len(batch_imgs) else []
        max_width = max((img.width for img in col_imgs), default=0)
        col_widths.append(max_width)
    
    row_heights = []
    for r in range(rows):
        row_imgs = batch_imgs[r*cols:(r+1)*cols] if r*cols < len(batch_imgs) else []
        max_height = max((img.height for img in row_imgs), default=0)
    ...
    for fname in os.listdir(test_dir):
        if fname.lower().endswith(('.png', '.jpg', '.jpeg', '.bmp')):
            ...
            try:
                ...
                print(f"加载图片 {fname} 失败: {e}")
    
    if len(images) < 6:
        print(f"警告: 只找到 {len(images)} 张图片，无法进行完整测试")
        return False
